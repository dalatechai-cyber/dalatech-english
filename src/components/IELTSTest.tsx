      ...
      for (let i = 0; i < total; i += BATCH_SIZE) {
        ...
        await Promise.all(
          batch.map(async (turn, j) => {
            ...
            const idx = i + j
            const trimmed = typeof turn.text === 'string' ? turn.text.trim() : ''
            // Skip empty/whitespace turns — firing the API for them produces
            // bare 400 "Missing text" responses and clutters logs.
            if (!trimmed) {
              urls[idx] = null
              return
            }
            try {
              const url = await loadWithRetry(trimmed, turn.speaker)
              urls[idx] = url
            } catch {
              urls[idx] = null
            }
